...
def extract_and_sort(folder_name):
    ...
    files_list = converter.get_all_files(folder_path, folder_name)
...
def open_file():
    ...
    foldername = tkFileDialog.askdirectory(initialdir="/", title='Please select a training folder')

    # Clean and fill the entry with filename
    entry.delete(0, END)
    entry.insert(0, foldername)

    length = len(foldername.split('/'))-1
    folder_path = '/'.join(foldername.split('/')[0:length]) + '/'

    foldername = foldername.split('/')[length]
    return foldername
