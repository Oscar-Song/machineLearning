...
def prepare_csv(area_str, good_folder,bad_folder):
    good_folder = good_folder.split('/')[len(good_folder.split('/'))-1]
    bad_folder = bad_folder.split('/')[len(bad_folder.split('/'))-1]

    raise_frame(histogram)
    root.geometry("550x450")
    area = areas.index(area_str)
    good_file_list = converter.get_all_files(path,good_folder)
    ...
    plotButton.grid(row=1,column=1)
    ...
    '''
    def make_csv_hist(area, good_file_list, bad_file_list, golden_stat):
        z_plots_good = []
        z_plots_bad = []

        nn = autoencoder.load(path+'/areas/area_'+str(area))
        ...
        for i in range(len(good_file_list)):
            ...
            test_batchnorm = batch_list_Test(test_list[area], golden_stat[area])
    ...
    threading.Thread(target=lambda: make_csv_hist(area,good_file_list,bad_file_list,golden_stat)).start()
...
def plot_hist(area_str,threshold_str, lower_bound_str = "0", upper_bound_str = "10000"):
    if threshold_str == "":
        ...
        def plot_histogram(area, threshold, lower_bound = 0, upper_bound = 10000):
            ...
            false_pos_lab.grid(row=4,column=0)
            false_neg_lab.grid(row=5,column=0)
            ...
            plot_widget.grid(row=6, column=0)

        threading.Thread(target=lambda: plot_histogram(area,threshold)).start()
...
'''
def start_prog(foldername,area_str,mini_batch_size_str,epoch_str,eta_str,lm_str,train_number_str,validation_number_str,test_number_str,dropout_ratio_str):

    #Output error message when necessary
    if area_str == '[Select area to show]' or mini_batch_size_str == '' or epoch_str == '' or eta_str == '' or lm_str == '' or train_number_str =='':
        tkMessageBox.showerror("Error", "Complete all field!")
    else:
        ...
        validation_number = int(validation_number_str)
        ...
        def train_autoencoder(foldername, area,mini_batch_size,epoch,eta,lm,train_number,validation_number,test_number,dropout_ratio):
            ...
            net = Network([
                ...
                mini_batch_size)
            #net = ourNN.Network([31,10,31],cost = ourNN.CrossEntropyCost)

            results = net.SGD(training_data, epoch, mini_batch_size, eta, validation_data, test_data,lmbda=lm, monitor =True)
            #net.SGD(train_batch[0], 10, 10, 0.25, lmbda=0.0, monitor_training_cost=True)

            progress_var.set(4)
            lab_progress.config(text="Saving parameters...")

            net.save(path+'/areas/area_'+str(area))
        ...
        threading.Thread(target=lambda: train_autoencoder(foldername, area,mini_batch_size,epoch,eta,lm,train_number,validation_number,test_number,dropout_ratio)).start()    # Multi-threading to avoiding GUI freeze
...
Label(setting,text="Threshold(Optional. eg:0.315)").grid(row=9, column=0, sticky='e')
threshold_entry = Entry(setting, width=10)
threshold_entry.grid(row=9,column=1,padx=2,pady=2,sticky='we',columnspan=10)


#Allow user to set lower bound from an entry
Label(setting,text="Lower bound(Optional. eg:0)").grid(row=10, column=0, sticky='e')
lower_entry = Entry(setting, width=10)
lower_entry.grid(row=10,column=1,padx=2,pady=2,sticky='we',columnspan=10)

#Allow user to set upper bound from an entry
Label(setting,text="Upper bound(Optional. eg:1000)").grid(row=11, column=0, sticky='e')
upper_entry = Entry(setting, width=10)
upper_entry.grid(row=11,column=1,padx=2,pady=2,sticky='we',columnspan=10)
...
command=lambda:start_prog(folder_entry.get(),var.get(),mini_entry.get(),epoch_entry.get(),eta_entry.get(),lm_entry.get(),train_entry.get(),valid_entry.get(),test_entry.get(),dropout_entry.get())).grid(row=12, column=10, sticky='ew', padx=8, pady=4)

#Progress bar for training

Label(prog,text="").grid(row=0,column=0)
Label(prog,text="").grid(row=1,column=0)
Label(prog,text="").grid(row=2,column=0)
Label(prog,text="").grid(row=3,column=0)
...
Button(result, text="Prepare CSVs for histogram", command=lambda: prepare_csv(var.get(),good_hist_entry.get(),bad_hist_entry.get())).grid(row=17,column=0,sticky='ew', padx=10, pady=10)
...
plotButton = Button(histogram, text="Plot histogram",command=lambda:plot_hist(var.get(),threshold_entry.get()), state="disabled" )
platBackButton = Button(histogram,text="Train Again",command=get_back).grid(row=2,column=0,sticky='ew', padx=10, pady=10)
false_pos_lab = Label(histogram, text="")
false_neg_lab = Label(histogram, text="")
